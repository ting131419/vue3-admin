<template>
    ...
    <div class='table'>
        ...
        <el-pagination class="mt-4" size="small" background layout="prev, pager, next" :total="config.total"
            ...
            <el-row>
                <el-col :span="12">
                    <el-form-item label="性别" prop="sex" :rules="[{required:true, message:'性别是必选项'}]">
                        <el-select v-model="formUser.sex" placeholder="请选择" style="width:150px" >
            ...
            proxy.$refs.userForm.validate(async (valid)=>{
                if(valid){
                    if(action.value=='add'){
                        ...
                        //（自己加的）将时间格式转换为yyyy-MM-dd格式，才能在日期组件中显示
                        formUser.birth = timeFormat(formUser.birth);
        ...
        const handleDelete = (row) =>{
            ElMessageBox.confirm('确定删除吗？')
                .then(async () => {
                    ...
                    //为什么这里要重新获取数据呢？  因为不重新获取数据的话，删除了数据id的新数据串不会显示
                    getUserData(config)
                })
                .catch(() => {
                    // catch error
